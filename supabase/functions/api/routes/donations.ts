...
export async function handleDonationRoute(
  ...
  // ============================================================================
  // TEMPORARY / TESTING-ONLY: POST /donations/monthly/dev-mock-subscribe
  // Stripe's React Native SDK doesn't run inside Expo Go (only built apps).
  // This bypass lets Stephanie test the held-funds + home-banner + redirect
  // flow end-to-end in Expo Go by creating a real DB monthly_donation +
  // a single mock "charged" transaction without ever touching Stripe.
  // REMOVE THIS BEFORE THE NEXT APP STORE SUBMISSION.
  // ============================================================================
  if (method === "POST" && route === "/donations/monthly/dev-mock-subscribe") {
    if (!userId) {
    ...
    try {
      const body = await req.json();
      const beneficiaryId = parseInt(body.beneficiary_id ?? body.beneficiaryId, 10);
      const amount = parseFloat(body.amount);
      const heldForDonorChoiceRaw =
        body.held_for_donor_choice === true || body.heldForDonorChoice === true;
      ...
      // Only honor the held flag when the beneficiary is THRIVE.
      ...
      if (heldForDonorChoiceRaw) {
        const { data: chosen } = await supabase
          ...
          .eq("id", beneficiaryId)
          .maybeSingle();
        heldForDonorChoice = !!chosen?.is_thrive;
      }

      const fakeSubId = `sub_devmock_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      ...
      const { data: donation, error: dErr } = await supabase
        ...
        .single();
      if (dErr) {
        console.error("dev-mock-subscribe donation insert error:", dErr);
        return new Response(
          JSON.stringify({ error: dErr.message || "Could not create mock donation" }),
          { headers: { "Content-Type": "application/json" }, status: 500 },
        );
      }

      // Mock "first charge" transaction so held-balance + the home banner
      // and redirect flow have something to work with.
      await supabase.from("transactions").insert({
        user_id: userId,
        type: "monthly_donation",
        amount,
        description: `[Dev mock] Monthly donation to beneficiary ${beneficiaryId}`,
        reference_id: `inv_devmock_${Date.now()}`,
      ...
      // Sync the user's monthly_donation amount + selected beneficiary,
      // matching the real subscribe path.
      ...
      await supabase
        ...
        .update({
          ...
          preferences: {
            ...
            preferredCharity: beneficiaryId,
            donationAmount: amount,
          },
        })
        .eq("id", userId);

      console.log("⚠️ [DEV] Created mock subscription for user", userId, "amount", amount, "held=", heldForDonorChoice);
      return new Response(
        JSON.stringify({
          success: true,
          dev_mock: true,
      ...
      console.error("dev-mock-subscribe threw:", e);
      return new Response(
