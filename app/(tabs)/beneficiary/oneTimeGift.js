...
export default function OneTimeGiftScreen() {
  ...
  const handleContinue = async () => {
    ...
    const beneficiaryIdForPayment = await resolveCheckoutBeneficiaryId({
      params,
      selectedBeneficiary,
    });
    ...
    try {
      ...
      const response = await API.createOneTimePaymentSheet({
        beneficiary_id: beneficiaryIdForPayment,
      ...
      Alert.alert(
        "Success",
        `Your one-time gift of $${donationAmount.toFixed(2)} has been completed.`,
        [{ text: "OK", onPress: () => router.replace("/(tabs)/menu") }],
      );
    } catch (error) {
      console.error("❌ One-time gift payment failed:", error);
      Alert.alert(
        "Error",
        error.message || "Failed to process one-time gift. Please try again.",
      );
    } finally {
      ...
      >
          ...
          {beneficiaryImage ? (
            ...
            <View
              style={[
                ...
                styles.beneficiaryImagePlaceholder,
              ]}
            >
        ...
        <TouchableOpacity
          style={[
            ...
            (!amount || selectedAmount < 1 || isProcessing) &&
              styles.continueButtonDisabled,
          ]}
          onPress={handleContinue}
          disabled={!amount || selectedAmount < 1 || isProcessing}
        >
          <Text style={styles.continueButtonText}>Continue to Checkout</Text>
        </TouchableOpacity>
        {/* Info Note */}
        <Text style={styles.infoNote}>
          💝 Your one-time gift will be processed securely and added to your
          transaction history.
        </Text>
      </ScrollView>
    </KeyboardAvoidingView>
